---
name: travel-planner
description: 테마와 장소 기반으로 최적의 여행 코스를 설계하고, 검증된 식당/카페/활동을 5개 이상의 옵션으로 제공. 사용자가 "여행 계획", "여행 코스", "데이트 코스", "맛집 투어", "테마 여행", "단체 여행", "워크샵 일정" 등을 요청할 때 사용. naver-map-verify 스킬과 연동하여 Playwright MCP로 장소 더블 검증 수행.
---
...
### Step 3.1: 🗺️ 네이버지도 더블 검증 (Playwright MCP)

> **웹 검색 결과를 네이버지도에서 실제 존재/영업 여부 재확인**

```
Playwright MCP 실행:
  1. browser_navigate: https://map.naver.com/p/search/{location} {장소명}
  2. browser_wait_for: 2초 (검색 결과 로딩)
  3. browser_snapshot: 검색 결과 캡처
...
추출 정보:
  ...
  - 영업 상태 (영업중/영업종료 + 오픈시간)
  - 별점 및 리뷰 수
  - TV 방송 출연 정보 ← 핫플 자동 감지!
  - 네이버지도 직접 링크 (place_id 기반)
...
> **Step 3.1 네이버지도 검증 + 웹 검색 결과를 종합하여 핫플레이스 제외 판정**

```
자동 제외 조건 (하나라도 해당 시 메인 추천 제외):

  [네이버지도 기반 - Step 3.1에서 감지]
  - 🗺️ TV방송정보 섹션 존재 (THE맛있는녀석들, 생활의달인, 백종원 등)
  - 🗺️ 방문자 리뷰 10,000개 이상
  - 🗺️ 블로그 리뷰 5,000개 이상
...
제외 시 처리:
  1. 해당 장소 메인 추천에서 제외
  2. "⚠️ 핫플레이스 주의" 라벨 부착 + TV출연 프로그램명 표시
...
- 👥 수용: {인원}
- 💰 가격: {가격대}
- ⭐ 평점: {별점} ({리뷰수}개 리뷰) ← 네이버지도 검증
- 🚶 혼잡도: {한산/보통/혼잡} | 추천 시간: {X시}
- 💡 **추천 이유**: {왜 이 시간대에 이 장소인지 구체적 설명}
- 🔗 [네이버지도](https://map.naver.com/p/entry/place/{place_id}) | [다이닝코드](url)
...
□ 🗺️ 네이버지도 더블 검증 완료 (Step 3.1)
   - Playwright로 장소 존재 여부 확인했는가?
   - 영업 상태/시간 네이버지도에서 재확인했는가?
   - 네이버지도 직접 링크 확보했는가?
...
| 1.4.0 | 2025-01 | **네이버지도 더블 검증 (Step 3.1) 추가** - Playwright MCP로 장소 존재/영업 확인, TV출연 자동 감지, 네이버지도 직접 링크 제공 |
